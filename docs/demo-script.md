...
```
Dashboard scenario selector on **steady**, speed **8x**, policies LRU / LFU / GDSF / AACMS.
...
> "AACMS scores every object by a **multi-factor value** — access pattern, size,
...
- Point at **Cumulative cost** chart: "Same workload, same cache size, same cost
  model for all four. AACMS — the red line — is already the cheapest."
...
- **Hit rate** chart: "LRU and LFU dip — they're evicting the working set to make
  room for one-hit-wonders. AACMS's admission control refuses to cache low-value
  objects, so its hit rate holds."
- **Cost** chart: "The gap *widens* during the spike — that's the whole point,
  AACMS is most valuable exactly when things go wrong."
- **AACMS cache capacity** chart: "And the autoscaler added capacity here —
...
> GDS and GDSF. AACMS cuts cost 59–62% versus the best baseline. Note LFU
> actually gets *worse* than LRU under a popularity shift — stale frequency
> counts — while AACMS stays flat. And this is the same engine, no retuning, on
...
No — the fixed-capacity number (`AACMS-fixed` row) still beats every baseline at
...
Then AACMS ≈ GDSF by construction (the bandit tilt goes to ~0). You lose nothing;
