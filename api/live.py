...
from workload.real_catalog import build_real_catalog
...
class LiveSim:
    def __init__(self, scenario: str = "steady", profile: str = "api", *,
        ...
        self.catalog = build_real_catalog() if profile == "real" else build_catalog(profile, seed=seed)
        ...
        # the real catalog's objects are tiny (a few hundred bytes of JSON) —
        # the synthetic 3 MiB floor would let the whole thing fit in L1 at
        # once, with no eviction pressure and nothing to actually decide.
        floor_bytes = 2 * 1024 if profile == "real" else 3 * 1024 * 1024
        l1 = max(int(working * 0.12), floor_bytes)
